				     ...
				     int packet, int status)
{
	...
	if (packet < 0)
		printk(KERN_WARNING "URB status %d [%s].\n",
				status, errmsg);
	else
		printk(KERN_INFO "URB packet %d, status %d [%s].\n",
...
void stk1160_copy_video(struct stk1160 *dev, u8 *src, int len)
{
	...
	if ((unsigned long)dst + lencopy >
		...
		printk(KERN_WARNING "stk1160: buffer overflow detected\n");
		return;
	}
	...
	while (remain > 0) {
		...
		if (lencopy < 0) {
			printk(KERN_WARNING "stk1160: negative lencopy detected\n");
			return;
		}
