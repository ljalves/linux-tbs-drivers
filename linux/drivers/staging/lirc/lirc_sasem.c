...
static void deregister_from_lirc(struct sasem_context *context)
{
	...
	if (retval)
		printk("%s: unable to deregister from lirc (%d)",
			__func__, retval);
	else
...
static int vfd_open(struct inode *inode, struct file *file)
{
	...
	if (context->vfd_isopen) {
		printk("%s: VFD port is already open", __func__);
		retval = -EBUSY;
	} else {
...
static int vfd_close(struct inode *inode, struct file *file)
{
	...
	if (!context->vfd_isopen) {
		printk("%s: VFD is not open", __func__);
		retval = -EIO;
	} else {
				...
				size_t n_bytes, loff_t *pos)
{
	...
	if (!context->dev_present) {
		printk("%s: no Sasem device present", __func__);
		retval = -ENODEV;
		goto exit;
	}
...
static int ir_open(void *data)
{
	...
	if (context->ir_isopen) {
		printk("%s: IR port is already open", __func__);
		retval = -EBUSY;
		goto exit;
	}
	...
	if (retval)
		printk("%s: usb_submit_urb failed for ir_open (%d)",
		    __func__, retval);
	else {
		context->ir_isopen = 1;
			...
			const struct usb_device_id *id)
{
	...
	if (vfd_ep_found) {
		...
		if (!tx_urb) {
			printk("%s: usb_alloc_urb failed for VFD urb",
			    __func__);
			alloc_status = 6;
			goto alloc_status_switch;
		}
