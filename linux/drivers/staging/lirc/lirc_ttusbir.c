...
static int set_use_inc(void *data)
{
	...
	for (i = 0; i < num_urbs; i++) {
		...
		if (retval) {
			printk("%s: usb_submit_urb failed on urb %d",
			    __func__, i);
			return retval;
		}
	}
...
static int probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (ttusbir->alt_setting != -1)
		...
		printk("Could not find alternate setting\n");
		kfree(ttusbir);
		return -EINVAL;
	}
	...
	if (lirc_buffer_init(&ttusbir->rbuf, sizeof(int), 256) < 0) {
		printk("Could not get memory for LIRC data buffer\n");
	...
	if (ttusbir->minor < 0) {
		printk("Error registering as LIRC driver\n");
		usb_set_intfdata(intf, NULL);
	...
	for (i = 0; i < num_urbs; i++) {
		...
		if (!ttusbir->urb[i]) {
			printk("Could not allocate memory for the URB\n");
			for (j = i - 1; j >= 0; j--)
				kfree(ttusbir->urb[j]);
			lirc_buffer_free(&ttusbir->rbuf);
