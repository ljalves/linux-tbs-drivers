...
static int display_open(struct inode *inode, struct file *file)
{
	...
	if (!interface) {
		printk("%s: could not find interface for minor %d",
		    __func__, subminor);
		retval = -ENODEV;
		goto exit;
	}
	...
	if (!context) {
		printk("%s: no context found for minor %d",
					__func__, subminor);
		retval = -ENODEV;
		goto exit;
	}
	...
	if (!context->display) {
		printk("%s: display not supported by device", __func__);
		retval = -ENODEV;
	} else if (context->display_isopen) {
		printk("%s: display port is already open", __func__);
		retval = -EBUSY;
	} else {
...
static int display_close(struct inode *inode, struct file *file)
{
	...
	if (!context) {
		printk("%s: no context for device", __func__);
		return -ENODEV;
	}
	...
	if (!context->display) {
		...
		printk("%s: display is not open", __func__);
		retval = -EIO;
	} else {
...
static int send_packet(struct imon_context *context)
{
	...
	if (retval) {
		...
		printk("%s: error submitting urb(%d)", __func__, retval);
	} else {
		...
		retval = wait_for_completion_interruptible(
				&context->tx.finished);
		if (retval)
			printk("%s: task interrupted", __func__);
		mutex_lock(&context->ctx_lock);

		retval = context->tx.status;
		if (retval)
			printk("%s: packet tx failed (%d)", __func__, retval);
	}
			 ...
			 size_t n_bytes, loff_t *pos)
{
	...
	if (!context->dev_present) {
		printk("%s: no iMON device present", __func__);
		retval = -ENODEV;
		goto exit;
	}

	if (n_bytes <= 0 || n_bytes > IMON_DATA_BUF_SZ - 3) {
		printk("%s: invalid payload size", __func__);
		retval = -EINVAL;
		goto exit;
	}
	...
	do {
		...
		if (retval) {
			printk("%s: send packet failed for packet #%d",
					__func__, seq/2);
			goto exit;
		} else {
			seq += 2;
		      ...
		      const struct usb_device_id *id)
{
	...
	if (!context) {
		printk("%s: kzalloc failed for context", __func__);
		alloc_status = 1;
	...
	if (!ir_ep_found) {
		printk("%s: no valid input (IR) endpoint found.", __func__);
		retval = -ENODEV;
		alloc_status = 2;
		goto alloc_status_switch;
	}
	...
	if (!driver) {
		printk("%s: kzalloc failed for lirc_driver", __func__);
		alloc_status = 2;
		goto alloc_status_switch;
	}
	rbuf = kmalloc(sizeof(struct lirc_buffer), GFP_KERNEL);
	if (!rbuf) {
		printk("%s: kmalloc failed for lirc_buffer", __func__);
		alloc_status = 3;
		goto alloc_status_switch;
	}
	if (lirc_buffer_init(rbuf, BUF_CHUNK_SIZE, BUF_SIZE)) {
		printk("%s: lirc_buffer_init failed", __func__);
		alloc_status = 4;
		goto alloc_status_switch;
	}
	rx_urb = usb_alloc_urb(0, GFP_KERNEL);
	if (!rx_urb) {
		printk("%s: usb_alloc_urb failed for IR urb", __func__);
		alloc_status = 5;
		goto alloc_status_switch;
	}
	tx_urb = usb_alloc_urb(0, GFP_KERNEL);
	if (!tx_urb) {
		printk("%s: usb_alloc_urb failed for display urb",
		    __func__);
		alloc_status = 6;
		goto alloc_status_switch;
	}
	...
	if (lirc_minor < 0) {
		printk("%s: lirc_register_driver failed", __func__);
		alloc_status = 7;
		goto unlock;
	} else
	...
	if (retval) {
		printk("%s: usb_submit_urb failed for intf0 (%d)",
		    __func__, retval);
		mutex_unlock(&context->ctx_lock);
		goto exit;
	}
...
static int __init imon_init(void)
{
	...
	if (rc) {
		printk("%s: usb register failed(%d)", __func__, rc);
		return -ENODEV;
	}
